class CarMaintenancesController < ApplicationController
  ...
  def index
    query = @car.car_maintenances.apply_filter(keyword: params[:q], categories: params[:categories])
    @car_maintenances = query.order(performed_at: :desc).paginate(page: params[:page], per_page: 20)
  ...
  def destroy
    ...
    redirect_to car_car_maintenances_path, status: :see_other, notice: "#{t('views.item.car_maintenance')} #{t('views.flash.notice_deleted')}."
  end
