...
export default class extends Controller {
  ...
  connect() {
    console.log('carsell--upload connected')
    this.list = []
  }
  ...
  async upload(e) {
    ...
    const totalImagesInDB = info.car_sell_images.length
    const totalToBeUploaded = e.target.files.length
    if ((totalImagesInDB + totalToBeUploaded) > 10 ){
      console.log("can not upload more than 10 images")
      return;
    }
  ...
  async showImage(json) {
    let response = await fetch(json.car_sell_image.show_url, {
      method: 'GET',
      headers: {
